...
#include "../inst/include/affineCF.h"
using namespace std;
...
arma::cube affineCFderivsEvalCpp(const arma::cube coeffs, const arma::mat stateMat){
  ...
  unsigned int U = coeffs.n_rows;
  unsigned int T = coeffs.n_cols;
  unsigned int N = stateMat.n_cols;
  ...
  // Calculate cf
  arma::cube cfVals = affineCFevalCpp(coeffs_cf, stateMat, false);
  
  // Extract first derivative coeffs
  arma::cube coeffs_d1 = coeffs.subcube(0,0,1*Np1,U-1,T-1,2*Np1-1);
  
  // calculate derivative
  arma::cube cfD1Vals_log = affineCFevalCpp(coeffs_d1, stateMat, true);
  ...
  arma::cube cfD2Vals_log = affineCFevalCpp(coeffs_d2, stateMat, true);
  ...
  arma::cube cfD3Vals_log = affineCFevalCpp(coeffs_d3, stateMat, true);
  arma::cube cfD3Vals = cfVals % (cfD3Vals_log + 3.0*cfD1Vals_log % cfD2Vals_log + arma::pow(cfD1Vals_log,3.0));
